...
async function main() {
  ...
  const nova = new BedrockNovaVisionClient({ region: process.env.AWS_REGION ?? "us-east-1", modelId: process.env.BEDROCK_MODEL_ID ?? "global.amazon.nova-2-lite-v1:0" });
  ...
  const stop = async () => {
    ...
    try { await preview.cleanup(); } catch { console.error("Ring cleanup failed; session may remain active until upstream expiry."); process.exitCode = 1; }
...
main().catch(() => { console.error("Preview setup failed. Set RING_API_BASE_URL, RING_DEVICES_PATH and RING_ACCESS_TOKEN, then retry from the repository root."); process.exitCode = 1; });
